...
/* ===== メニュー自動生成 ===== */
const excludeTitles = ['基本プラン','設立＋LPパック','設立+LPパック','フルサポートパック']; // 子アコーディオンはメニューに出さない
...
function buildMenu(){
  ...
  sections.forEach(sec=>{
    ...
    // 見出し（h2）がある場合だけ、h4を作る。ただし plans は出さない。
    ...
    if (sec.id === 'plans') {
      // 等間隔にするためのクラス（CSS ① を足してあります）
      wrap.classList.add('compact');

      // ★もし「料金プラン」という薄いラベルを出したい場合は下３行のコメントを外す
      // const lab = document.createElement('div');
      // lab.className = 'menu-separator';
      // lab.textContent = '料金プラン';
    ...
    details.forEach(d=>{
      ...
      if (excludeTitles.some(x => t.includes(x))) return; // 子タイトルは非表示
